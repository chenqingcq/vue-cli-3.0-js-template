<template>
  <div class="container-statistics-staff">
    <div class="visitor-list">
      <div class="title center">
        <div class="text">
          <span>{{$t('statistics.staff.recent')}}</span>
        </div>
        <div class="more" @click="watchMore(2,0)">{{$t('statistics.staff.more')}}</div>
      </div>
      <div class="content">
        <template v-if="staffList.length">
          <li v-for="(item ,index) in staffList" :key="`${index}-visitor-list`">
            <img
    ...
    <div class="panel">
        ...
        <div class="operation">
          <div class="time">
            <el-date-picker
              ...
              @change="queryStaffRecordStatistics"
            ></el-date-picker>
      ...
      <div class="content">
        <!-- <div class="item top">
          ...
          <div id="statistics-state"></div>
        </div>-->
        <div class="item middle">
          <div class="title center">
            <img :src="accessIcon" alt @click="queryStaffRecordStatistics" />
...
import { queryStaffRecordStatistics } from "@/api/statistics";
...
export default {
  ...
  methods: {
    ...
    watchMore(index, subIndex) {
      this.$router.push(`/main/staff/list?currentIndex=1`, () => {
    ...
    handleSelectChange(show) {
      if (!show) return;
      this.queryStaffRecordStatistics();
    },
    ...
    async report() {
      try {
        ...
        if (res.code == 1 && res.data) {
          let { url } = res.data;
          url && this.download(url, "staffAccessRecord-excel");
        } else {
    ...
    async queryStaffRecordStatistics() {
      try {
        let formData = new FormData();
        formData.append("companyId", this.companyId);
        formData.append("groupId", this.groupId);
        ...
        const res = await queryStaffRecordStatistics(formData, {
          headers: { "Content-Type": "multipart/form-data" }
        });
        if (res.code == 1) {
          console.log(res);
    ...
    async getRecentRecord(type = "YYYY-MM-DD HH:mm:ss") {
      ...
      try {
        ...
        if (res.code == 1 && res.data) {
          console.log(res);
          this.staffList = res.data.slice(0, 6);
        } else {
          this.$Message.error(this.$t("common.searchError"));
          this.staffList = [];
        }
  ...
  mounted() {
    ...
    this.queryStaffRecordStatistics();
    this.$ebus.$on("/main/statistics/staff", () => {
      this.getRecentRecord();
      this.queryStaffRecordStatistics();
    });
    window.addEventListener("resize", () => {
      if (this.$route.path.includes("/main/statistics/staff")) {
